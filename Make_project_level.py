...
#Creates the header only from the VCF file (standard)
...
def print_dataframe(nrows,d, arg_list ):
	...
	df.index.name = "SNPID"
	df.sort_index(axis=0).to_csv(path_or_buf='OUTPUT_MERGED_VCF', sep=',',header=True, index=True)
...
def usage():
	print "This is now how its done. Dont be lazy! Please input at least 1 VCF file\n"
	print "==================== Use Me! ===================================\n"
	print "        Make_project_level.py 1012_test.vcf 1013_test.vcf \n"
	print "Please check the file OUTPUT_MERGED_VCF which has the final output"
	print "======================= END ================================\n"
	exit()
